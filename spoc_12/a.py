...
class scheduler:
    ...
    def load(self, program_description):
        ...
        # if proc_id == 0:
        #     self.proc_info[0][PROC_CODE] = [DO_IO, DO_IO, DO_YIELD, DO_COMPUTE, DO_YIELD]
        # else:
            # self.proc_info[1][PROC_CODE] = [DO_YIELD, DO_IO, DO_YIELD, DO_YIELD, DO_YIELD]
    ...
    def next_proc(self, pid=-1):
        ...
        if pid == -1:
            ...
            i = pid
        l = range(i + 1 , self.get_num_processes()) + range(0 , i + 1)
        for j in l:
            if self.proc_info[j][PROC_STATE] == STATE_READY:
                # self.proc_info[self.curr_proc][PROC_STATE]
                self.curr_proc = j
                self.move_to_running(0)
                # print "change " + str(self.curr_proc)
                return
    ...
    def run(self):
        ...
        while self.get_num_active() > 0 :
            ...
            temp_io_done = 0
            for pid in range(len(self.proc_info)):
                if clock_tick + 1 in self.io_finish_times[pid]:
                    self.move_to_ready(0, pid)
                    temp_io_done = 1
            ...
            if instruction_to_execute == DO_YIELD:
                self.move_to_ready(0, self.curr_proc)
                self.next_proc()
                # pass #YOU should delete this
